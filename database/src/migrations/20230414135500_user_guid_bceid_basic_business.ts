...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(`
    ALTER TABLE
      "system_user"
    ADD
      user_guid varchar(200)
    DEFAULT
      null;
  `);

  // Add unique index constraint for user_guid column
  await knex.raw(`
  ...
  await knex.raw(`
    ...
    UPDATE system_user SET user_guid = 'restoration_api' WHERE user_identifier LIKE 'restoration_api';
  `);
  ...
  await knex.raw(`
    ...
    begin
      ...
      delete from restoration_context_temp where tag = 'user_id';
